...
// Wraps a StubHTTPClient and runs a closure on the first `data(from:limit:)` call,
// i.e. mid-flight during metadata.fetch, so tests can simulate a concurrent store
// edit that lands while enrichment is still awaiting the network.
private struct InterceptingHTTPClient: HTTPClient {
...
@Test func concurrentRefileDuringFetchIsNotReverted() async throws {
    ...
    let client = InterceptingHTTPClient(inner: stub) {
        store.refile(id: bookmark.id, to: "Articles")
    }
    let service = EnrichmentService(
        store: store,
